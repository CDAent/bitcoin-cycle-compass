"""
Automated tests for Bitcoin Cycle Compass v8.4 Sprint 2A snapshot service.
...
def _seed_snapshot(db_path, d, overrides=None):
    ...
    fields = {
        ...
        'scores_quality': 'live',
        'updater_version': '8.4.0-dev', 'sprint': '2A',
    }
    if overrides:
...
class TestSnapshotCreation(unittest.TestCase):
    ...
    def test_updater_version_stored(self):
        s = snapshot('2024-06-01', self.db)
        self.assertEqual(s['updater_version'], '8.4.0-dev')

    def test_sprint_stored(self):
        s = snapshot('2024-06-01', self.db)
        self.assertEqual(s['sprint'], '2A')
...
class TestBuildMetadata(unittest.TestCase):
    ...
    def test_app_version_stored(self):
        ...
        self.assertEqual(meta['appVersion'], '8.4.0-dev')

    def test_sprint_stored(self):
        meta = get_build_metadata(self.db)
        self.assertEqual(meta['sprint'], '2A')
